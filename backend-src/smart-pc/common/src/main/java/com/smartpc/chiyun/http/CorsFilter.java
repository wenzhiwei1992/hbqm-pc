...
public class CorsFilter implements Filter {
	...
	protected void handleSimpleCORS(final HttpServletRequest request,
		...
		if (!(requestType == CORSRequestType.SIMPLE || requestType == CORSRequestType.ACTUAL)) {
			throw new IllegalArgumentException(
					new MessageFormat(
							"Expects a HttpServletRequest object of type [{0}] or [{1}]")
							.format(new Object[] {
	...
	protected void handlePreflightCORS(final HttpServletRequest request,
		...
		if (requestType != CORSRequestType.PRE_FLIGHT) {
			throw new IllegalArgumentException(new MessageFormat(
					"Expects a HttpServletRequest object of type [{0}]")
					.format(new Object[] {
							CORSRequestType.PRE_FLIGHT.name()
							.toLowerCase() },
							new StringBuffer(), null).toString());
		}
	 ...
	 */
	public static final String DEFAULT_PREFLIGHT_MAXAGE = "7200";
