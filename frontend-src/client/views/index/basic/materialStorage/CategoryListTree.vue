<template>
  <div>
    <el-header ref="headOperationBar" style="height:30px;padding:0;">
      <el-row>
    ...
    <el-container>
      <el-aside
        width="300px"
        id="tab"
        tyle="overflow-y: auto;overflow-x: auto"
      >
        <el-tree
          style="width:450px;"
          ref="vuetree"
    ...
    >
      <category-edit-tree
        :editForm="editForm"
        :mode="mode"
        @category-finish="categoryFinish"
        @category-cancel="categoryCancel"
      >
      </category-edit-tree>
    </el-dialog>
  </div>
...
export default {
  ...
  components: {
    'category-edit-tree': categoryEditTree,
  },
  data() {
    // let data = [{
    //     id: 0,
  ...
  methods: {
    ...
    list() {
      categoryModel.getCategorysToTree().then((data) => {
    ...
    handleNodeClick(data, node) {
      categoryModel.findCategoryByParentId(data.id).then((data1) => {
        this.categorys = data1.entity
        console.log('dataTest------>', data)
        this.addCategoryId = data.id
      })
    },
    mouseenter(node) {
    ...
    append(data) {
      ...
      //0为前端页面显示第一级'构件类型'
      if (data.id != 0) {
        categoryModel.categoryLoad(data.id).then((data1) => {
          //console.log('data1---->',data1)
          this.editForm.parentId = data1.entity.id
          this.editForm.parentName = data1.entity.categoryName
          this.editFormVisible = true
          //console.log('parentName------>',this.editForm.parentName)
        })
      } else {
        this.editFormVisible = true
      }
    ...
    remove(node, data) {
      this.$confirm('此操作将永久删除该构件类型, 是否继续?', '提示', {
        ...
        categoryModel.delete(node.data.id).then(data => {
    ...
    addClick() {
      ...
      if (this.addCategoryId != 0) {
        categoryModel.categoryLoad(this.addCategoryId).then((data1) => {
          this.editForm.parentId = data1.entity.id
          this.editForm.parentName = data1.entity.categoryName
          this.editFormVisible = true
        })
    ...
    editClick(row) {
      this.mode = this.GLOBAL.STATUS.UPDATE
      categoryModel.categoryLoad(row.id).then((data1) => {
        this.editForm = data1.entity
        this.editFormVisible = true
      })
    },
    deleteClick(row) {
      this.$confirm('此操作将永久删除该构件类型, 是否继续?', '提示', {
        ...
        categoryModel.delete(row.id).then(data => {
          if (data.status == this.GLOBAL.STATUS.FAIL) {
            Notification({
              message: data.msg,
              type: 'warning'
    ...
    deletesClicks() {
      ...
      this.$confirm('此操作将永久删除勾选原料类别, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
    ...
    handleDrop(draggingNode, dropNode, dropType, ev) {
      ...
      categoryModel.updateCategoryCodeLevelByDrag(model).then((data) => {
        this.list()
        let info = {
          id: data.entity
        }
    ...
    allowDrop(draggingNode, dropNode, type) {
      if (type == 'prev' && dropNode.data.label == '原料类型') {
        return type !== 'prev';
      } else if (type == 'next' && dropNode.data.label == '原料类型') {
        return type !== 'next';
      } else {
        return true;
      }
    ...
    allowDrag(draggingNode) {
      return draggingNode.data.label.indexOf('原料类型') === -1;
    },
    categoryFinish() {
      //取消前重新渲染页面
      this.editForm = {
