          ...
          <el-col :span="8">
            <div class="grid-content bg-purple">
              <el-form-item
                ...
                prop="orgId"
              >
                <el-select v-model="editForm.orgId">
                  <el-option
                    v-for="item in orgs"
                    :key="item.id"
                    :label="item.orgName"
        ...
        >
          <el-col>
            ...
            >
              ...
              >
                <el-select
                  v-model="orderProductCondition.houseTypeId"
                  placeholder="户型"
                  collapse-tags
                  clearable
                  @change="bindBuildData(orderProductCondition.houseTypeId)"
                >
                  <el-option
                    :key="item.id"
                    :value="item.id"
  ...
  methods: {
    ...
    deletesProduct() {
      ...
      this.selected.forEach((row) => {
        ...
        let orderDetailId = row.orderDetailId;
        // 检查 detail 是否在生产，如果在，则不允许删除，如果不在，则直接删掉
        orderModel.deleteProductDetailByOrderDetailId(orderDetailId).then(data => {
          if (data.status == 'success') {
            Notification({
              type: 'success',
              message: '删除成功'
      ...
      orderModel.findAllByFormInline(this.formInline).then(data => {
        if (data.entity.content) {
          data.entity.content.forEach(val => {
            val.dateCreated = val.dateCreated.slice(0, 10)
          })
        }
