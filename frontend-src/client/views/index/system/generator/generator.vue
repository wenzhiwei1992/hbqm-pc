<template>
  <div>
    <el-tooltip
      ...
      placement="top"
            >
            <el-button type="primary" icon="el-icon-finished" size="mini" @click="toGen"></el-button>
    </el-tooltip>
    <!-- <el-button type="primary" size="mini" @click="toGen">生成代码</el-button> -->
    <el-dialog :visible.sync="dialog" :close-on-click-modal="false" :before-close="cancel" title="代码生成配置" append-to-body width="70%">
...
export default {
  ...
  methods: {
    toGen() {
      this.dialog = true
      generator.getConfig().then(data=>{
          this.form = data
          this.form.cover = this.form.cover.toString()
      })
      generator.getByTableName(this.name).then(data => {
    ...
    doSubmit() {
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          generator.updateGenConfig(this.form).then(data=>{
            generator.generator(this.data, this.name).then(res => {
              this.$notify({
                title: '生成成功',
                type: 'success',
                duration: 2500
